...
import { validateEmail } from "../_utilities/helperFunctions";
import { sendContactInfo } from "../_utilities/action";
      ...
      <div className="flex flex-col justify-center col-span-4 md:col-span-6">
        ...
        <ul className="flex flex-col gap-6">
          ...
          <li className="flex gap-3 items-center">
            ...
            <p className="text-neutral-600 text-base font-medium">hello@abstractly.com</p>
          </li>
        </ul>
      </div>
      {/* form */}
          ...
          <div className="relative flex flex-col gap-1.5 w-full">
            ...
            <div className="relative">
              <input
                ...
                className={`text-sm text-neutral-900 w-full bg-neutral-50 border border-neutral-200 rounded px-3.5 py-[9px] placeholder:text-sm placeholder:text-neutral-500 focus:outline-none focus:border-0 ${
                  errors.name ? "focus:shadow-inset-error-focus border-red-300" : "focus:shadow-inset-focus"
                }`}
                type="text"
                name="name"
                placeholder="Your name"
              />
              {errors.name && <span className="absolute left-0 bottom-[-24px] text-sm text-red-600">{errors.name}</span>}
            </div>
          </div>
          {/* email */}
          <div className="flex flex-col gap-1.5 w-full">
            ...
            <div className="relative">
              <input
                ...
                className={`text-sm text-neutral-900 w-full bg-neutral-50 border border-neutral-200 rounded px-3.5 py-[9px] placeholder:text-sm placeholder:text-neutral-500 focus:outline-none focus:border-0 ${
                  errors.email ? "focus:shadow-inset-error-focus border-red-300" : "focus:shadow-inset-focus"
                }`}
                type="text"
                name="email"
                placeholder="example@example.com"
              />
              {errors.email && <span className="absolute left-0 bottom-[-24px] text-sm text-red-600">{errors.email}</span>}
            </div>
          </div>
        </div>
        {/* message */}
