...
import { subscribeNewsletter } from "../_utilities/action";
      ...
      <div className="col-span-4 md:col-span-6 h-[564px] md:h-[460px]">
        ...
        <form action={handleSubmit} className="flex flex-col gap-4 md:flex-row">
          <div className="relative flex flex-col gap-3 md:gap-4">
            <span className={`${errorMessage === "" && "hidden"} absolute top-[-25px] text-sm text-red-600`}>{errorMessage}</span>
            <input
              value={email}
              name="email"
              onChange={(e) => setEmail(e.target.value)}
              placeholder="Enter your email"
              className={`w-full md:w-[334px] rounded bg-neutral-50 border border-neutral-200 px-3.5 py-[9px] text-sm placeholder:text-sm focus:outline-none focus:border-0 text-neutral-900 placeholder:text-neutral-500${
                errorMessage === "" ? " focus:shadow-inset-focus" : " border-red-300 focus:shadow-inset-error-focus"
              }`}
            />
